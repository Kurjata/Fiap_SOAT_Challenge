...
import com.fiap.soat.controllers.ChargeController;
import com.fiap.soat.entities.request.charge.ChargeCreateRequest;
import com.fiap.soat.presenters.ChargePresenter;
...
public class ChargeApi {
  ...
  public Mono<Void> create(@RequestBody @Valid ChargeCreateRequest request) {
    return Mono.just(request)
        .map(this.chargePresenter::toDTO)
        .flatMap(this.chargeController::create)
        .then();
  }
  ...
  public Mono<Void> payment(@RequestBody @Valid ChargeCreateRequest request) {
    return Mono.just(request)
        .map(this.chargePresenter::toDTO)
        .flatMap(this.chargeController::payment)
        .then();
  }
}
